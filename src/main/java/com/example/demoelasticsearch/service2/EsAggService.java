package com.example.demoelasticsearch.service2;
...
public class EsAggService {
    ...
    public void getTest() throws IOException {
        SearchRequest searchRequest = new SearchRequest("bidding_simple_list");//创建搜索请求
        SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder();
        ...
        String termsAggName = "my_terms";//指定聚合的名称
        //定义terms聚合，指定字段为城市
        TermsAggregationBuilder termsAggregationBuilder = AggregationBuilders.terms(termsAggName).field("biddingPattern");
        DateRangeAggregationBuilder dateAgg = AggregationBuilders.dateRange("date_agg").field("createAt");
        //dateAgg.format("yyyy-MM-dd HH:mm:ss");
        dateAgg.addUnboundedTo("all_time", "now+1d/d-1s");
        dateAgg.addRange("two_week", "now-2w/d","now+1d/d-1s");
        dateAgg.subAggregation(termsAggregationBuilder);

        searchSourceBuilder.aggregation(dateAgg);//添加聚合
        searchRequest.source(searchSourceBuilder);//设置查询请求
        //执行查询
        SearchResponse searchResponse = client.search(searchRequest, RequestOptions.DEFAULT);
        Aggregations aggregations = searchResponse.getAggregations();//获取聚合结果

        Range ranges = aggregations.get("date_agg");//获取聚合返回的对象

        for (Range.Bucket bucket : ranges.getBuckets()) {
            String bucketKey = bucket.getKeyAsString();//获取桶名称
            long docCount = bucket.getDocCount();//获取文档个数
            Terms t = bucket.getAggregations().get(termsAggName);
            for (Terms.Bucket tBucket : t.getBuckets()) {
                long docCount1 = tBucket.getDocCount();
                String keyAsString = tBucket.getKeyAsString();
                System.err.println(bucketKey+":"+docCount+"----------------------"+keyAsString+":"+docCount1+"--------------------------");
